...
#define LOCTEXT_NAMESPACE "VaRestful_BreakJson"
...
public:
	...
	virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		...
		UClass *Class = Cast<UClass>(StaticLoadObject(UClass::StaticClass(), NULL, TEXT("class'VaRestfulPlugin.VaRestfulJsonObject'")));
	...
	FBPTerminal* RegisterInputTerm(FKismetFunctionContext& Context, UVaRestful_BreakJson* Node)
	{
		// Find input pin
		UEdGraphPin* InputPin = NULL;
		for (int32 PinIndex = 0; PinIndex < Node->Pins.Num(); ++PinIndex)
	...
	{
		UVaRestful_BreakJson* Node = Cast<UVaRestful_BreakJson>(InNode);
 ...
 * Main node class
 */
UVaRestful_BreakJson::UVaRestful_BreakJson(const FObjectInitializer &ObjectInitializer)
	: Super(ObjectInitializer)
{
}

FNodeHandlingFunctor* UVaRestful_BreakJson::CreateNodeHandler(class FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_BreakJson(CompilerContext);
}

void UVaRestful_BreakJson::AllocateDefaultPins()
{
...
FLinearColor UVaRestful_BreakJson::GetNodeTitleColor() const
{
	return FLinearColor(255.0f, 255.0f, 0.0f);
}

void UVaRestful_BreakJson::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	bool bIsDirty = false;
...
void UVaRestful_BreakJson::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UVaRestful_BreakJson::GetMenuCategory() const
{
...
void UVaRestful_BreakJson::CreateProjectionPins(UEdGraphPin *Source)
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
	UClass *Class = Cast<UClass>(StaticLoadObject(UClass::StaticClass(), NULL, TEXT("class'VaRestfulPlugin.VaRestfulJsonObject'")));

	for (TArray<FVaRestful_NamedType>::TIterator it(Outputs); it; ++it)
	{
#if ENGINE_MINOR_VERSION >= 19
		FName Type;
#else
		...
		switch ((*it).Type) {
			case EVaRestful_JsonType::JSON_Bool:
				Type = K2Schema->PC_Boolean;
				break;

			case EVaRestful_JsonType::JSON_Number:
				Type = K2Schema->PC_Float;
				break;

			case EVaRestful_JsonType::JSON_String:
				Type = K2Schema->PC_String;
				break;

			case EVaRestful_JsonType::JSON_Object:
...
FText UVaRestful_BreakJson::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return LOCTEXT("VaRestful_Break_Json.NodeTitle", "Break Json");
}
...
public:
	...
	virtual void RegisterNets(FKismetFunctionContext& Context, UEdGraphNode* InNode) override
	{
		UVaRestful_MakeJson* Node = Cast<UVaRestful_MakeJson>(InNode);
		FNodeHandlingFunctor::RegisterNets(Context, Node);
...
* Main node class
*/
UVaRestful_MakeJson::UVaRestful_MakeJson(const FObjectInitializer &ObjectInitializer)
	: Super(ObjectInitializer)
{
}

FNodeHandlingFunctor* UVaRestful_MakeJson::CreateNodeHandler(class FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_MakeJson(CompilerContext);
}

void UVaRestful_MakeJson::AllocateDefaultPins()
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
...
FLinearColor UVaRestful_MakeJson::GetNodeTitleColor() const
{
	return FLinearColor(255.0f, 255.0f, 0.0f);
}

void UVaRestful_MakeJson::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	bool bIsDirty = false;
...
void UVaRestful_MakeJson::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
	// actions get registered under specific object-keys; the idea is that
	// actions might have to be updated (or deleted) if their object-key is
	// mutated (or removed)... here we use the node's class (so if the node
...
FText UVaRestful_MakeJson::GetMenuCategory() const
{
	static FNodeTextCache CachedCategory;
...
void UVaRestful_MakeJson::CreateProjectionPins(UEdGraphPin *Source)
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
	UClass *Class = Cast<UClass>(StaticLoadObject(UClass::StaticClass(), NULL, TEXT("class'VaRestfulPlugin.VaRestfulJsonObject'")));

	for (TArray<FVaRestful_NamedType>::TIterator it(Inputs); it; ++it)
	{
		FName Type;
		UObject *Subtype = nullptr;
		FString FieldName = (*it).Name;
...
FText UVaRestful_MakeJson::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return LOCTEXT("VaRestful_Make_Json.NodeTitle", "Make Json");
}
